...
document
    ...
    .addEventListener('change', function () {
        ...
        i.addEventListener(
            'load',
            function () {
                o.src = i.result; // Preview the image
            },
            false
        );
        ...
        if (e) {
            ...
            xhr.open('POST', '/admin/upload-profile-image', true);
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    ...
                    toastr.error('An error occurred while processing your request.'); // Generic error message
                    console.error('Request failed with status:', xhr.status); // Log the HTTP status
                }
            };
